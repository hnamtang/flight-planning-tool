from ..routing import create_arc, create_rhumb_line


class CFLegGenerator:
    def __init__(self, leg_data, num_interp_pnts):
        """
        Generate Course to Fix leg: Rhumb line path with constant magnetic course.
        ...
        leg_data : dict
            ...
            the following keys:
                ...
                * `flyby_center_lat`, `flyby_center_lon` : float
                    Latitude and longitude of the center arc [deg]. None if `is_flyby`
                    is False
                * `flyby_start_lat`, `flyby_start_lon` : float
                    Latitude and longitude of the arc start [deg]. None if `is_flyby`
                    is False
                * `flyby_end_lat`, `flyby_end_lon` : float
                    Latitude and longitude of the arc end [deg]. None if `is_flyby`
                    is False
                * `crs` : float
                    A course given by aviation authority for the CF leg [deg]
                * `leg_type` : str
    ...
    def generate(self, return_distance=False):
        ...
        if return_distance:
            straight_part, distance_straight = create_rhumb_line(
                lat_from,
                lon_from,
                lat_to,
                lon_to,
            ...
            straight_part = create_rhumb_line(
                lat_from,
                lon_from,
                lat_to,
                lon_to,
